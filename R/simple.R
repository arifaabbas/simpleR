...
#'view the dataset and variables, detect missing data, get summary statistics, remove/keep variables, rename variables,
...
simple <- function(){
  e.simple <- new.env(parent = emptyenv())
  e.simple$df <- NULL
...
Let's begin!", sep = "")
  ...
  back <- function(x, backto, df = e.simple$df, dfname = e.simple$dfname){
    if(x == "!!!"){
      backto}
    else if(x == "!!!main"){
  ...
  options <- function(option, validoptions, backto, df = e.simple$df, dfname = e.simple$dfname,
                      variables = e.simple$variables, name = e.simple$name){
    repeat{
...
Do you want to:
  ...
  varname <- function(dothis, backto, solo = 0, df = e.simple$df, dfname = e.simple$dfname){
    e.simple$variables <- NULL
    if(solo == 1){
      repeat{
...
> If the dataframe you would like to use is not in your global environment,
  ...
  df_import <- function(df = e.simple$df, dfname = e.simple$dfname, name = e.simple$name){
    ...
    if(!(is.null(name))){
      e.simple$dfname <- name
      e.simple$df <- import()
      assign(e.simple$dfname[1], e.simple$df, envir = .GlobalEnv)
      main()
    }
  }
  import <- function(){
